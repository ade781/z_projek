...
const EditMisi = () => {
    ...
    useEffect(() => {
        const fetchMisi = async () => {
            try {
                ...
                const response = await axios.get(`${BASE_URL}/misi/${id}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });

                const data = response.data.data || response.data;
                setForm({
                    judul_misi: data.judul_misi,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`${BASE_URL}/misi/${id}`, form, {
                headers: { Authorization: `Bearer ${token}` },
            });
            ...
            setTimeout(() => {
                setTimeout(() => {
                    navigate(`/detail-misi-owner/${id}`);
                }, 1500);

            }, 1500);
        } catch (error) {
            console.error("Gagal mengedit misi:", error.response?.data || error.message);
            setErrorMsg(error.response?.data?.message || "Gagal mengedit misi.");
        } finally {
        ...
        <div className="min-h-screen bg-gradient-to-b from-blue-50 to-blue-100 py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-bold text-blue-800 mb-2">Edit Misi</h1>
                    <p className="text-blue-600">
                        Perbarui detail misi untuk petualang
                    </p>
                </div>
